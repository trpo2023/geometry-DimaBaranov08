...
int check_figure(const char *s)
{
    ...
    if (strcmp(tmp, figure) != 0) return 0;
    return 1;
}

int left_bracket(const char *s)
{
    for (int i = 0; i < ) 
}

void print_error(const char *s, char *x, char *y, char *rad)
{
    int error = 0;
    if (!check_figure(s)) 
    {
...
{
    char x[10], y[10], rad[10];
    print_error(s, x, y, rad);
}
